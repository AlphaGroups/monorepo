...
const StudentClasses = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ✅ your Tabs + Class Cards + Assignments section remains same,
          just replaced react-router Link with next/link */}

      {/* Example fix inside class cards */}
      {classes.map((classItem) => (
        <Card key={classItem.id} className="hover-lift card-elegant">
          <CardHeader className="pb-3">
            <div className="flex items-center space-x-3">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="pt-2 border-t">
              <div className="flex items-center justify-between">
                ...
                <Link href={`/student/classes/${classItem.id}`} passHref>
                  <Button size="sm" className="bg-gradient-primary">
                    <Play className="mr-1 h-3 w-3" />
                    Continue
                  </Button>
